...
class VirtualEnvManager:
    ...
    def __init__(self, venv_path: str, requirements_file: str):
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Setup a Python virtual environment and install requirements."
    )
    parser.add_argument(
        "--venv",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Duration in seconds to wait before exiting on failure (default: 2)",
    )
...
if __name__ == "__main__":
    ...
    venv_manager = VirtualEnvManager(
        venv_path=args.venv, requirements_file=args.requirements
    )
    venv_manager.run(duration=args.duration)